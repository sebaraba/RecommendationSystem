...
class Recommender(object):
    ...
    def loadDatasets(self):
        ...
        buyers =[]
        items =[]
        datasetsList1 = ['test_csv.csv', 'data.csv', 'trainingDataset1.csv']
        datasetsList2 = ['events_sorted.csv']

        with open('../data/' + self.dataset) as csvfile:
            readCSV = csv.reader(csvfile, delimiter=',')
            next(readCSV, None)
            for row in readCSV:
                if self.dataset in datasetsList1:
                    if row[1] != '' and row[6] != '' and row[1] != ' ' and row[6] != ' ':
                        buyers.append(row[6])
                        items.append(row[1])
                if self.dataset in datasetsList2:
    ...
    def computationOnItemNodes(self, bestSimilarityScores):
        ...
        with open('../data/testDataset1.csv') as csvfile:
            ...
            for row in readCSV:
                if row[1] != '' and row[6] != '' and row[1] != ' ' and row[6] != ' ':
                    actualItems.append(row[1])
                    actualBuyers.append(row[6])
        
        actualEdges = zip(actualBuyers, actualItems) 
        ...
        print('Number of predictions:', len(predictionsList))
        good = 0
        bad = 0
        accuracy = 0
        for entry in predictionsList:
            if entry in actualEdges:
                good += 1
            else:
                bad += 1

        print('Good predictions:', good)
